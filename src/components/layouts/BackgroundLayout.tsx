...
import BG_COLORFUL_LINES from "../../assets/backgrounds/bg-colorful-lines.svg";
...
export const BackgroundLayout: React.FC<Props> = ({ theme = "login", mask, children, imgProps, ...props }) => {
  const getTheme = () => {
    switch (theme) {
      case "login":
        return "bg-white";
  ...
  return (
    <div className={`h-screen m-auto relative flex items-center ${getTheme()}`} {...props}>
      <img
        ...
        src={getImg()}
        alt="back"
        className={`absolute w-full h-full object-cover ${imgProps?.className}`}
      />
